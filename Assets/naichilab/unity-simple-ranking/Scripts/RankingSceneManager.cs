...
namespace naichilab
{
	public class RankingSceneManager : MonoBehaviour
	{
		...
		/// <summary>未記入時の名前</summary>
		[SerializeField] private string defaultEntryName = "no name";
		...
		void Start ()
		{
			...
			this.nameInputField.onValueChanged.AddListener((text) => {
		...
		IEnumerator GetHighScoreAndRankingBoard ()
		{
			...
			//ハイスコア取得
			{
				...
				if (hiScoreCheck.Count > 0) {
					...
					if(highScoreSpreadSheetObject ["name"].ToString().Equals(defaultEntryName)){
			...
			if (this.highScoreSpreadSheetObject == null) {
				...
				this.highScore = highScore.Value;

				if (RankingLoader.Instance.setting.Order == ScoreOrder.OrderByAscending) {
					//数値が低い方が高スコア
					sendScore = score.Value < highScore.Value;
				} else {
					//数値が高い方が高スコア
					sendScore = highScore.Value < score.Value;
				}
			}
		...
		private IEnumerator SendScoreEnumerator ()
		{
			...
			if(RankingLoader.Instance.Score.Value < highScore){
				this.highScoreSpreadSheetObject ["hiscore"] = highScore;
			}
			...
			if(RankingLoader.Instance.Score.Value < highScore){
				this.highScoreLabel.text = highScore.ToString();
			}else{
